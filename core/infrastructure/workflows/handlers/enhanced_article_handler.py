...
class EnhancedArticleHandler(WorkflowHandler):
    ...
    def prepare_context(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(
            ...
            for keyword in ["finance", "investment", "market", "trading"]
        ):
        ...
        if any(
            keyword in topic.lower()
            for keyword in ["technology", "ai", "software", "digital"]
        ):
    ...
    def post_process_task(
        ...
        if task_id == "task1_brief":
            ...
            if any(
                word in task_output.lower()
                for word in ["statistic", "data", "number", "%"]
            ):
                context["includes_data"] = True

            logger.info(
                f"🔍 Research completed with {context.get('research_depth')} depth"
            )
            ...
            logger.info(
                f"📝 Content created: {content_length} words (target: {target_count})"
            )
        ...
        try:
            ...
            if task_outputs:
                ...
                if task4_output:
                    ...
                    logger.info(
                        f"📄 Selected final content from {task4_output[0]} ({task4_output[2]} chars) - compliance reviewed"
                    )
                elif task3_output:
                    final_content = task3_output[1]
                    logger.info(
                        f"📄 Selected final content from {task3_output[0]} ({task3_output[2]} chars) - pre-compliance"
                    )
                else:
                    ...
                    logger.info(
                        f"📄 Selected final content from {task_outputs[0][0]} ({task_outputs[0][2]} chars) - fallback"
                    )
            else:
