...
class ReoplaEnhancedArticleWithImageHandler(WorkflowHandler):
    ...
    def validate_inputs(self, context: Dict[str, Any]) -> None:
        ...
        if len(topic) < 5:
            raise ValueError("Topic must contain at least 5 characters for meaningful generation")
        ...
        if target_word_count:
            ...
            if count < 400:
                raise ValueError("target_word_count must be at least 400 words for Reopla articles")
            if count > 2000:
                raise ValueError("target_word_count must not exceed 2000 words for this workflow")
    ...
    def post_process_task(self, task_id: str, task_output: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if task_id == 'task1_reopla_brief':
            ...
            logger.info("🔍 Research dossier prepared (%s characters)", context['research_characters'])
            ...
            if payload:
                ...
                logger.warning("⚠️ Unable to parse JSON payload from image specialist output")
        return context

    def post_process_workflow(self, context: Dict[str, Any]) -> Dict[str, Any]:
        final_content = context.get('task4_reopla_compliance_output') or context.get('task3_reopla_draft_output', '')
