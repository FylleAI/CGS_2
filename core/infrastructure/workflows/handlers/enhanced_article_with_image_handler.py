        ...
        if task_id == 'task5_image_generation':
            ...
            if raw_payload:
                try:
                    ...
                    has_payload = bool(image_data.get('image_url') or image_data.get('image_data'))
                    context['image_generated'] = bool(image_data.get('success', False) and has_payload)
                    if not has_payload:
                        context['image_generation_failed'] = True
                        context.setdefault('image_generation_warning', 'Image tool executed but returned no image_url/image_data')
                    ...
                    context['image_generation_failed'] = True
                    context.setdefault('image_generation_warning', 'Unable to parse image generation output as JSON')
            else:
                logger.warning("⚠️ Image generation output is empty")
                context['image_generation_failed'] = True
                context.setdefault('image_generation_warning', 'No tool payload detected in agent output')
