...
class EnhancedArticleWithImageHandler(EnhancedArticleHandler):
    ...
    def validate_inputs(self, context: Dict[str, Any]) -> None:
        ...
        if style not in self._VALID_STYLES:
            raise ValueError(
                f"Image style must be one of: {sorted(self._VALID_STYLES)}"
            )
        ...
        if task_id == "task5_image_generation":
            ...
            if raw_payload:
                try:
                    ...
                    # Determine if we actually have an image payload
                    has_payload = bool(
                        image_data.get("image_url") or image_data.get("image_data")
                    )
                    context["image_generated"] = bool(
                        image_data.get("success", False) and has_payload
                    )
                    if not has_payload:
                        context["image_generation_failed"] = True
                        context.setdefault(
                            "image_generation_warning",
                            "Image tool executed but returned no image_url/image_data",
                        )
                        logger.warning("⚠️ Image tool returned no image_url/image_data")
                    else:
                        logger.info(
                            "🖼️ Image generation result captured (provider=%s)",
                            image_data.get("provider"),
                        )
                    ...
                    context.setdefault(
                        "image_generation_warning",
                        "Unable to parse image generation output as JSON",
                    )
            else:
                ...
                context.setdefault(
                    "image_generation_warning",
                    "No tool payload detected in agent output",
                )
    ...
    def _extract_tool_payload(output: str | None) -> str | None:
        ...
        match = re.search(
            r"\[image_generation_tool RESULT\](.*?)\[/image_generation_tool RESULT\]",
            output,
            re.DOTALL,
        )
        if match:
