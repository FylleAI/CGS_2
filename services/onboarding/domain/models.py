...
from packages.contracts.onboarding import CompanySnapshot as SnapshotContract
...
class OnboardingGoal(str, Enum):
    ...
    LINKEDIN_POST = "content_generation"      # Backwards compatibility alias
    NEWSLETTER = "content_generation"         # Backwards compatibility alias
...
class CompanySnapshot(BaseModel):
    ...
    def _compat_from_legacy(cls, data: Any) -> Any:
        """Allow initializing from legacy snapshot structure."""

        if not isinstance(data, dict):
            return data

        legacy_company = data.pop("company_info", None)
        if legacy_company and "company" not in data:
            data["company"] = CompanyInfo(
                name=legacy_company.get("name")
                or legacy_company.get("company_name", ""),
                legal_name=legacy_company.get("legal_name"),
                website=legacy_company.get("website"),
                industry=legacy_company.get("industry"),
                headquarters=legacy_company.get("headquarters"),
                size_range=legacy_company.get("size_range"),
                description=legacy_company.get("description", ""),
                key_offerings=legacy_company.get("key_offerings", []),
                differentiators=legacy_company.get("differentiators", []),
                evidence=[
                    Evidence(**item) if isinstance(item, dict) else item
                    for item in legacy_company.get("evidence", [])
                ],
            )

        legacy_audience = data.pop("audience_info", None)
        if legacy_audience and "audience" not in data:
            data["audience"] = AudienceInfo(
                primary=legacy_audience.get("persona_name")
                or legacy_audience.get("target_audience"),
                secondary=legacy_audience.get("secondary", []),
                pain_points=legacy_audience.get("pain_points", []),
                desired_outcomes=legacy_audience.get("goals", []),
            )

        legacy_voice = data.pop("voice_info", None)
        if legacy_voice and "voice" not in data:
            data["voice"] = VoiceInfo(
                tone=legacy_voice.get("tone"),
                style_guidelines=legacy_voice.get("style_guidelines", []),
                forbidden_phrases=legacy_voice.get("forbidden_phrases", []),
                cta_preferences=legacy_voice.get("cta_preferences", []),
            )

        legacy_insights = data.get("insights")
        if isinstance(legacy_insights, dict) and not isinstance(
            legacy_insights, InsightsInfo
        ):
            data["insights"] = InsightsInfo(
                positioning=legacy_insights.get("positioning"),
                key_messages=legacy_insights.get("key_messages", []),
                recent_news=legacy_insights.get("recent_news", []),
                competitors=legacy_insights.get("competitors", []),
            )
    ...
    def validate_question_ids(cls, v: List[ClarifyingQuestion]) -> List[ClarifyingQuestion]:
        """Ensure question IDs are unique when provided."""
        if not v:
            return v
    ...
    def to_contract(self) -> SnapshotContract:
        """Convert onboarding snapshot to shared contract representation."""

        return SnapshotContract.from_domain_snapshot(self)
...
class OnboardingResponse(BaseModel):
    """Response from services.onboarding operations."""
