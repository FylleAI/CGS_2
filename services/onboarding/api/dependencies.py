...
from services.onboarding.application.pipelines.card_export import CardExportPipeline
...
def get_card_export_pipeline() -> Optional[CardExportPipeline]:
    ...
    client = get_card_service_adapter()
    if not client:
        return None
    return CardExportPipeline(card_client=client)
...
def get_execute_onboarding_use_case() -> ExecuteOnboardingUseCase:
    ...
    return ExecuteOnboardingUseCase(
        ...
        card_export_pipeline=get_card_export_pipeline(),
    )
