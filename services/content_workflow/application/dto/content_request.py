...
class ContentGenerationRequest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.context is None:
            self.context = {}
        elif isinstance(self.context, WorkflowContext):
            self.context = self.context.to_dict()
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "generation_params": (
                self.generation_params.to_dict() if self.generation_params else None
            ),
            "custom_instructions": self.custom_instructions,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ContentGenerationRequest":
        ...
        return cls(
            ...
            provider_config=(
                ProviderConfig.from_dict(data["provider_config"])
                if data.get("provider_config")
                else None
            ),
            generation_params=(
                GenerationParams.from_dict(data["generation_params"])
    ...
    def get_workflow_context(self) -> WorkflowContext:
        """Return the workflow context as a shared contract instance."""

        if isinstance(self.context, WorkflowContext):
            return self.context
        if isinstance(self.context, dict):
            return WorkflowContext.from_raw(self.context)
