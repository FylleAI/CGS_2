...
from ..dto.content_request import (
...
from ...infrastructure.workflows.registry import (
...
class GenerateContentUseCase:
    ...
    def __init__(
        ...
        self.agent_executor = AgentExecutor(
            agent_repository, llm_provider, provider_config
        )
    ...
    async def execute(
        ...
        try:
            ...
            if self.tracker is not None:
                try:
                    ...
                    self.tracker.add_log(
                        run_id,
                        "INFO",
                        f"Started content generation for topic: {request.topic}",
                    )
                    if request.provider_config:
                        self.tracker.add_log(
                            run_id,
                            "INFO",
                            f"Provider: {request.provider_config.provider.value}",
                        )
                        if request.provider_config.model:
                            self.tracker.add_log(
                                run_id,
                                "INFO",
                                f"Model: {request.provider_config.model}",
                            )
            ...
            if not workflow_type or workflow_type == "None":
                ...
                logger.info(
                    f"🔧 No workflow_type specified, defaulting to: {workflow_type}"
                )
            ...
            if self.tracker is not None and run_id:
                self.tracker.add_log(
                    run_id, "INFO", f"Dynamic workflow '{workflow_type}' completed"
                )
            ...
            if self.tracker is not None and run_id:
                try:
                    ...
                    self.tracker.add_log(
                        run_id,
                        "INFO",
                        f"Content generation completed successfully in {execution_time:.2f}s",
                    )
                except Exception as track_err:
            ...
            try:
                if self.tracker is not None and "run_id" in locals() and run_id:
                    ...
                    self.tracker.add_log(
                        run_id, "ERROR", f"Content generation failed: {str(e)}"
                    )
    ...
    async def _execute_dynamic_workflow(
        ...
        try:
            ...
            if workflow_type not in available_workflows:
                ...
                logger.info(
                    f"📋 Available workflows: {list(available_workflows.keys())}"
                )
    ...
    async def _create_legacy_workflow(
        self, workflow_type: str, context: Dict[str, Any]
    ) -> Workflow:
    ...
    async def _create_enhanced_article_workflow(
...
LENGTH: {context.get('length', 'medium')} length article
    ...
    async def _create_content_from_dynamic_result(
        self, workflow_result: Dict[str, Any], request: ContentGenerationRequest
    ) -> Content:
        """
        ...
        logger.info(
            f"🔍 Final output length: {len(final_output) if final_output else 0}"
        )
        if final_output:
        ...
        content = Content(
            ...
            metadata={
                "workflow_type": request.workflow_type,
                "generation_params": (
                    request.generation_params.to_dict()
                    if request.generation_params
                    else {}
                ),
                "workflow_summary": workflow_result.get("workflow_summary", {}),
    ...
    async def _configure_workflow_agents(
        self, workflow: Workflow, request: ContentGenerationRequest
    ) -> None:
    ...
    async def _execute_workflow(
        ...
        try:
            ...
            context = {
                ...
                "workflow_type": request.workflow_type,
                "generation_params": (
                    request.generation_params.__dict__
                    if request.generation_params
                    else {}
                ),
            }
            ...
            if "final_output" in execution_result and execution_result["final_output"]:
                ...
                logger.info(
                    f"✅ Using handler-selected final output ({len(final_output)} chars)"
                )
                return final_output
            ...
            if task_outputs:
                ...
                final_output = list(task_outputs.values())[-1]
                logger.info(
                    f"⚠️ Using fallback final output ({len(final_output)} chars)"
                )
                return final_output
            else:
    ...
    def _build_generation_prompt(
        self, request: ContentGenerationRequest, context: str
    ) -> str:
    ...
    async def _create_content_from_result(
        self, result: str, request: ContentGenerationRequest, workflow_id: Optional[str]
    ) -> Content:
        ...
        content = Content(
            ...
            client_profile=request.client_profile,
            target_audience=(
                request.generation_params.target_audience
                if request.generation_params
                else ""
            ),
            topic=request.topic,
    ...
    async def _setup_enhanced_article_agents(
        self, workflow: Workflow, request: ContentGenerationRequest
    ) -> None:
    ...
    async def _create_enhanced_article_tasks(
        self, workflow: Workflow, request: ContentGenerationRequest
    ) -> None:
...
- Valida la rilevanza per il target audience
    ...
    async def _setup_newsletter_premium_agents(
        self, workflow: Workflow, request: ContentGenerationRequest
    ) -> None:
    ...
    async def _setup_default_agents(
        self, workflow: Workflow, request: ContentGenerationRequest
    ) -> None:
