...
class RAGTool:
    ...
    def __init__(
        ...
        except Exception as exc:  # pragma: no cover - missing env handled gracefully
            logger.warning(f"Supabase settings not available: {exc}")
            settings = None

        self.rag_base_dir = Path(
            settings.knowledge_base_dir if settings else "data/knowledge_base"
        )
